 ...
 */
class AdultesController extends AppController {
    ...
    public function ajouter($source='inscription',$idJoueur=null) {
        if($this->Auth->User('Admin') == 1) {
            ...
            if($source == 'nouvelUsager') {
                ...
                $this->request->data['Adulte']['DateCreation'] = date('Y-m-d');
                $this->request->data['Adulte']['DerniereModif'] = date('Y-m-d');
                if(!empty($this->request->data)) { 
                    $this->Adulte->save($this->request->data);
                    $idAdulte = $this->Adulte->id;

                    $this->redirect(array('controller' => 'users', 'action' => 'add', $idAdulte));
                }
                ...
                if($this->Adulte->save($this->request->data)) {
                    ...
                    $this->redirect(array('controller' => 'parents', 'action' => 'fiche', $idParent));
                } else {
                    echo 'Fail';
                    $this->redirect(array('controller' => 'parents', 'action' => 'recherche'));
                }
